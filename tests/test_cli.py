...
import rstms_ehandler
from rstms_ehandler import __version__
from rstms_ehandler.example import cli


def test_version():
    """Test reading version and module name"""
    assert rstms_ehandler.__name__ == "rstms_ehandler"
    assert __version__
...
def test_cli_exception(run):
    ...
    result = run(cmd, assert_exception=FileNotFoundError)
